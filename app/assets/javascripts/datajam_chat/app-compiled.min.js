(function(f,g,i){g("chat/common",[,"js!chat/plugins/underscore_mixins.js","js!chat/plugins/jquery.imagesloaded.js","js!chat/plugins/jquery.scrollTo-1.4.2-min.js","js!chat/plugins/moment.min.js"],function(){window.Datajam||(Datajam={});Datajam.Chat={Models:{},Views:{},Collections:{}};Datajam.Chat.csrf={csrf_param:f("meta[name=csrf-param]").attr("content"),csrf_token:f("meta[name=csrf-token]").attr("content")};f("document").bind("csrfloaded",function(){Datajam.Chat.csrf.csrf_token=f("meta[name=csrf-token]").attr("content")})});
g("chat/upload",["js!chat/plugins/jquery.form.js"],f.noop);g("chat/tweet",["js!//platform.twitter.com/widgets.js"],f.noop);g("chat/collections/message",["chat/common"],function(){var c=jQuery;Datajam.Chat.Collections.Message=Backbone.Collection.extend({add:function(b,d){_.isArray(b)?c.each(b,_.bind(function(a,b){b=this._clean(b);this.add_or_replace(b,d)},this)):(b=this._clean(b),this.add_or_replace(model,d));return this},add_or_replace:function(b,c){var a=this.get(b.id);a?b.updated_at>a.get("updated_at")&&
...
this.model.get("id")+"]");if(c.length){var b=b.find(".nav a[data-controls-modal="+c.attr("id")+"]").eq(0),c=b.find(".badge"),d=a(this.el).find("li").length;c.length||(c=b.append('<span class="badge"></span>').find(".badge"));c.html(d);0==d?c.hide():c.show()}}}})});i(["chat/common","chat/views/chat"],function(){Backbone.emulateHTTP=!0;Backbone.emulateJSON=!0;f(function(){var c=Datajam.Chat;f(".datajamChatThread").not(".moderator").each(function(){new c.Views.Chat({el:f(this)})});f(".datajamChatThread.moderator").each(function(){i(["chat/views/moderator_chat"],
_.bind(function(){new c.Views.ModeratorChat({el:f(this)})},this))});f("a[data-controls-modal]").one("click",function(){f("#"+f(this).attr("data-controls-modal")).find(".modal-chat-controls").each(function(){i(["chat/views/chat_controls"],_.bind(function(){new c.Views.ChatControls({el:f(this)})},this))})})})})})(jQuery,curl.define,curl);