module Datajam
  class Chat::ChatIdentityController < Chat::EngineController
      ...
      if current_user
        ...
        unless current_name.blank?
          ...
          if already_registered_users.any?
            ...
            if Chat.sessions.get(current_name).blank? or Chat.sessions.get(current_name) == session[:session_id]
              Chat.sessions.set current_name, session[:session_id]
    ...
    def destroy
      if Chat.sessions.get(params[:display_name]) == session[:session_id]
        Chat.sessions.del(params[:display_name])
      end
