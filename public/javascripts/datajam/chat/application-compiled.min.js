...
this.resume()},pause:function(){Datajam.debug("pausing");this.model.set({paused:true},{silent:true})},pollForContent:function(){Datajam.debug("polling...");if(this._timeout){clearTimeout(this._timeout);this._timeout=null}if(this._request){this._request.abort();this._request=null}this._request=this.collection.fetch(g.extend({add:true},this.model.get("ajaxOptions")));if(!this.model.get("paused"))this._timeout=setTimeout(this.pollForContent,this.model.get("interval"))},pollForOpenness:function(){Datajam.debug("polling for openness");
if(this._timeout){clearTimeout(this._timeout);this._timeout=null}this.model.fetch().success(_.bind(function(a){a.chat.is_open&&this.open(a);a.chat.is_archived&&this.archive(a);if(!a.chat.is_open&&!a.chat.is_archived){this.render();this._timeout=setTimeout(this.pollForOpenness,this.model.get("interval"))}},this)).error(this.error).complete(this.loaded)},prevPage:function(){if(this.collection._oldest_seen_page){Datajam.debug("prev page");var a=g.Deferred();this.collection.fetch(g.extend({add:true,url:this.collection._oldest_seen_page},
this.model.get("ajaxOptions")));return a.promise()}return true},removeMessage:function(a){(a=this.$el.find(".comments").find("li#message_"+a.id))&&a.remove()},render:function(){Datajam.debug("rendering");var d=_(this.model.toJSON()).extend(Datajam.csrf),b=_.template(c),g=_.template(e),f=_.template(a),i=_.template(h);if(!d._id){Datajam.debug("No chat id found, returning from render...");Datajam.debug(d);return this}if(!d.is_open&&!d.is_archived){this.$el.html(g(d));return this}if(!this.$el.children().not(".closed").length){this.$el.html("");
...
this.delegateEvents();return this},resume:function(){Datajam.debug("resuming");this.model.set({paused:false},{silent:true});this.pollForContent()},submit:function(a){a.preventDefault();this.$el.find("form textarea").eq(0).focus();if(!this.$el.find("form").eq(0).hasClass("disabled"))if(a=this.$el.find("textarea").val()){this.disableSubmit();message=new i.models.Message({text:a});message.url=this.model.get("_submit_url");message.save(null,{dataType:"json",success:_.bind(function(a){this.$el.find("textarea").val("");
a.is_public?this.model.set({interval:500},{silent:true}):this.flash({type:"info",message:"Your message is awaiting moderation."});this.pollForContent()},this),error:_.bind(function(a,b){var d;(d=JSON.parse(b.responseText).errors)&&d.text?this.flash({type:"error",message:"Text "+d.text[0]}):this.flash({type:"error",message:"There was a problem posting your message."})},this),complete:this.enableSubmit})}},uploadDialog:function(a){a.preventDefault();g(g(a.target).attr("href")).trigger("click")},uploadFile:function(a){g(a.target).val()&&
g(a.target).parents("form").ajaxSubmit({dataType:"json",success:_.bind(function(b){if(b&&b.url){var c=g(a.target).parents("form").prev("form").find("textarea");c.val(c.val()+location.protocol+"//"+location.host+b.url);g(a.target).val("");c.focus()}else this.flash({type:"error",message:"Upload failed. Accepted file types are png, jpg, gif."})},this)})}})});h("chat/views/chat_controls",["text!chat/templates/chat/controls.html","chat/init","chat/models/chat"],function(c){var e=jQuery;Datajam.Chat.views.ChatControls=
...
var b={is_open:this.model.get("is_open"),is_archived:this.model.get("is_archived")},a=null;_(this.statuses).each(function(c,e){_.isEqual(c,b)&&(a=e+1)},this);a&&this.$el.find("select").val(a);return this},submit:function(){console.log(this.model.isNew());var b=this.$el.find("select");(b=b.length?b.first().val():null)&&this.model.save(this.statuses[b-1]);return this}})});h("chat/views/incoming_message",["text!chat/templates/message/incoming.html","chat/init","chat/views/message","chat/models/message"],
function(c){var e=jQuery,b=Datajam.Chat;b.views.IncomingMessage=b.views.Message.extend({events:{"click .approve":"approve","click .reject":"reject"},initialize:function(){_.bindAll(this,"approve","empty","getImages","getLinks","loading","parentModel","parentView","reject","render","_url");this.model||(this.model=new b.models.Message);this.model.bind("change",this.render)},approve:function(a){a.preventDefault();this.loading();this.model.url=this._url();this.model.set({is_moderated:!0,is_public:!0},
{silent:!0});this.model.save().success(_.bind(function(){e("#"+this.$el.attr("id")).remove()},this)).error(function(){alert("There was an error approving this message.")})},empty:function(){this.$el.html("")},loading:function(){this.$el.addClass("loading")},reject:function(a){a.preventDefault();this.loading();this.model.url=this._url();this.model.set({is_moderated:!0,is_public:!1},{silent:!0});this.model.save().success(_.bind(function(){e("#"+this.$el.attr("id")).remove()},this)).error(function(){alert("There was an error rejecting this message.")})},
render:function(){var a=this.model.toJSON(),b=this.parentModel();a.text=this._strip_tags(a.text);a.text=this._linkify(a.text);a.text=this._imgify(a.text);a.text=this._spaceify(a.text);a.text=this._linebreaks(a.text);this.$el.attr("id","message_"+a._id).attr("data-timestamp",a.updated_at);b&&b.get("is_admin")&&this.$el.addClass("sunlight");this.$el.html(_.template(c,a));this.delegateEvents();return this}})});h("chat/views/rejected_message",["text!chat/templates/message/rejected.html","chat/init","chat/views/message",
"chat/models/message"],function(c){var e=jQuery,b=Datajam.Chat;b.views.RejectedMessage=b.views.Message.extend({className:"message rejected",events:{"click .approve":"approve","click .deleteAttachments":"deleteAttachments"},initialize:function(){_.bindAll(this,"approve","deleteAttachments","empty","getImages","getLinks","loading","parentModel","parentView","render","_url");this.model||(this.model=new b.models.Message);this.model.bind("change",this.render)},approve:function(a){a.preventDefault();this.loading();
this.model.url=this._url();this.model.set({is_moderated:!0,is_public:!0},{silent:!0});this.model.save().success(_.bind(function(){e("#"+this.$el.attr("id")).remove()},this)).error(function(){alert("There was an error approving this message.")})},deleteAttachments:function(a){a.preventDefault();var b=this.getImages(),c=[];_(b).each(function(a){c.push(_.last(a.split("/")))});c=_(c).compact();c.length&&e.ajax({url:"/chats/upload.json",type:"POST",dataType:"json",data:{_method:"delete",filenames:c},success:_.bind(function(a){if(a){var c=
this.model.get("text");_(b).each(function(a){c=c.replace(a,"")});this.model.url=this._url();this.model.set({text:c});this.model.save()}},this)})},empty:function(){this.$el.html("")},loading:function(){this.$el.addClass("loading")},render:function(){var a=this.model.toJSON(),b=this.parentModel();a.text=this._strip_tags(a.text);a.text=this._linkify(a.text);a.text=this._imgify(a.text);a.text=this._spaceify(a.text);a.text=this._linebreaks(a.text);this.$el.attr("id","message_"+a._id).attr("data-timestamp",
a.updated_at);b&&b.get("is_admin")&&this.$el.addClass("sunlight");this.$el.html(_.template(c,a));this.delegateEvents();return this}})});h("chat/views/moderator_chat","text!chat/templates/chat/show.html text!chat/templates/chat/closed.html text!chat/templates/chat/error.html text!chat/templates/common/flash.html chat/init chat/models/moderator_chat chat/models/message chat/views/incoming_message chat/views/rejected_message chat/collections/moderator_message".split(" "),function(c,e,b){var a=jQuery,
