...
prevPage:function(){if(this.collection._oldest_seen_page){Datajam.debug("prev page");var a=g.Deferred();this.collection.fetch(g.extend({add:true,url:this.collection._oldest_seen_page},this.model.get("ajaxOptions")));return a.promise()}return true},removeMessage:function(a){(a=this.$el.find(".comments").find("li#message_"+a.id))&&a.remove()},render:function(){Datajam.debug("rendering");var d=_(this.model.toJSON()).extend(Datajam.csrf),b=_.template(c),g=_.template(e),f=_.template(a),i=_.template(h);
if(!d._id){Datajam.debug("No chat id found, returning from render...");Datajam.debug(d);return this}if(!d.is_open&&!d.is_archived){this.$el.html(g(d));return this}if(!this.$el.children().not(".closed").length){this.$el.html("");this.$el.append(b(d));this.$el.find(".commentsClip").scroll(_.bind(function(){this.$el.trigger("chatWindow:scroll")},this))}this.$el.find("form, .archived").remove();d.is_open?this.model.get("display_name")?this.$el.find(".tip").after(i(d)):this.$el.find(".tip").after(f(d)):
this.$el.find(".tip").remove();d.is_archived&&this.$el.prepend('<p class="archived">This is an archived event, comments are no longer being accepted.</p>');this.model.get("_keep_focus")&&this.$el.find("textarea, input[type=text]").focus();this.delegateEvents();return this},resume:function(){Datajam.debug("resuming");this.model.set({paused:false},{silent:true});this.pollForContent()},submit:function(a){a.preventDefault();this.$el.find("form textarea").eq(0).focus();if(!this.$el.find("form").eq(0).hasClass("disabled"))if(a=
this.$el.find("textarea").val()){this.disableSubmit();message=new i.models.Message({text:a});message.url=this.model.get("_submit_url");message.save(null,{dataType:"json",success:_.bind(function(a){this.$el.find("textarea").val("");a.is_public?this.model.set({interval:500},{silent:true}):this.flash({type:"info",message:"Your message is awaiting moderation."});this.pollForContent()},this),error:_.bind(function(a,b){var d;(d=JSON.parse(b.responseText).errors)&&d.text?this.flash({type:"error",message:"Text "+
d.text[0]}):this.flash({type:"error",message:"There was a problem posting your message."})},this),complete:this.enableSubmit})}},uploadDialog:function(a){a.preventDefault();g(g(a.target).attr("href")).trigger("click")},uploadFile:function(a){g(a.target).val()&&g(a.target).parents("form").ajaxSubmit({dataType:"json",success:_.bind(function(b){if(b&&b.url){var c=g(a.target).parents("form").prev("form").find("textarea");c.val(c.val()+location.protocol+"//"+location.host+b.url);g(a.target).val("");c.focus()}else this.flash({type:"error",
