...
this.render);this.model.bind("change",this.handleChange);g.ajax({url:"/chats/identity.json",dataType:"json"}).success(_.bind(function(d){d.display_name&&this.model.set({display_name:d.display_name,is_admin:d.is_admin})},this));this.pollForOpenness()},addMessage:function(d){var a=this.$el.find(".commentsClip"),b=a.find(".comments"),c=a.scrollTop(),g=b.height(),f=b.find("li"),e=new i.views.Message({model:d});this.model.set({interval:this.$el.attr("data-interval")},{silent:true});f.length&&this.collection.indexOf(d)<
f.length?f.eq(this.collection.indexOf(d)).before(e.render().$el):b.append(e.render().$el);this.model.get("_scroll_anchored")||b.find("img").imagesLoaded(_.bind(function(){var d=b.height()-g;a.scrollTo(c+d)},this))},archive:function(d){Datajam.debug("archiving");d&&this.bootstrap(d);d&&this.pollForContent();this.$el.removeClass("closed").removeClass("open").addClass("archived");this.model.set({is_open:false,is_archived:true});this.pause()},bootstrap:function(d){Datajam.debug("bootstrapping");if(this.collection){this.collection._oldest_seen_page=
this.collection.url;this.collection.reset()}else{this.collection=new i.collections.Message;this.collection.bind("add",this.addMessage);this.collection.bind("remove",this.removeMessage);this.collection.ajaxOptions=d.ajaxOptions;this.collection.url=this.model.url.replace(".json","/pages/"+d.page._id+".json");this.collection._newest_seen_page=this.collection.url;this.collection._oldest_seen_page=d.page.prev_page;this.collection.view=this;this.model.collection=this.collection}this.$el.children(".commentsClip").trigger("scroll")},
close:function(){Datajam.debug("closing");this.model.set({is_open:false,is_archived:false});this.$el.removeClass("archived").removeClass("open").addClass("closed");this.pollForOpenness()},destroy:function(){this.pause();this.$el.html("")},destroyIdentity:function(d){d.preventDefault();this.model.get("display_name")&&g.ajax({url:"/chats/identity.json",type:"post",dataType:"json",data:{display_name:this.model.get("display_name"),_method:"delete"}});this.model.set({display_name:null})},disableSubmit:function(){this.$el.find("form").addClass("disabled")},
enableSubmit:function(){this.$el.find("form").removeClass("disabled")},error:function(){this.$el.html(_.template(b,{}))},flash:function(d){d=g(_.template(f,d));this.$el.find("form .well").eq(0).append(d);d.hide().fadeIn().delay(4E3).fadeOut(function(){g(this).remove()})},handleBlur:function(d){this._focusTimeout=setTimeout(_.bind(function(){this.model.set({_keep_focus:false},{silent:true});g(d.target).parents(".datajamChatThread").removeClass("active")},this),1500)},handleChange:function(){Datajam.debug("model changed");
!this.model.get("is_open")&&!this.model.get("is_archived")&&this.pollForOpenness()},handleFocus:function(d){this._focusTimeout&&clearTimeout(this._focusTimeout);this.model.set({_keep_focus:true},{silent:true});g(d.target).parents(".datajamChatThread").addClass("active")},handleKeyDown:function(d){d.keyCode===13&&(d.ctrlKey||d.metaKey)&&g(d.target).parents("form").submit()},handleScroll:function(){var d,a;d=this.$el.find("div.commentsClip");a=d.find(".comments");d.scrollTop()===0?this.model.set({_scroll_anchored:true},
...
this.resume()},pause:function(){Datajam.debug("pausing");this.model.set({paused:true},{silent:true})},pollForContent:function(){Datajam.debug("polling...");if(this._timeout){clearTimeout(this._timeout);this._timeout=null}if(this._request){this._request.abort();this._request=null}this._request=this.collection.fetch(g.extend({add:true},this.model.get("ajaxOptions")));if(this.$el.children().length===0){Datajam.debug("something went wrong. rescuing...");this.render().collection.reset()}if(!this.model.get("paused"))this._timeout=
setTimeout(this.pollForContent,this.model.get("interval"))},pollForOpenness:function(){Datajam.debug("polling for openness");if(this._timeout){clearTimeout(this._timeout);this._timeout=null}this.model.fetch().success(_.bind(function(a){a.chat.is_open&&this.open(a);a.chat.is_archived&&this.archive(a);if(!a.chat.is_open&&!a.chat.is_archived){this.render();this._timeout=setTimeout(this.pollForOpenness,this.model.get("interval"))}},this)).error(this.error).complete(this.loaded)},prevPage:function(){if(this.collection._oldest_seen_page){Datajam.debug("prev page");
var a=g.Deferred();this.collection.fetch(g.extend({add:true,url:this.collection._oldest_seen_page},this.model.get("ajaxOptions")));return a.promise()}return true},removeMessage:function(a){(a=this.$el.find(".comments").find("li#message_"+a.id))&&a.remove()},render:function(){Datajam.debug("rendering");var d=_(this.model.toJSON()).extend(Datajam.csrf),b=_.template(c),g=_.template(e),f=_.template(a),i=_.template(h);if(!d._id){Datajam.debug("No chat id found, returning from render...");return this}if(!d.is_open&&
!d.is_archived){this.$el.html(g(d));return this}if(!this.$el.children().not(".closed").length){this.$el.html("");this.$el.append(b(d));this.$el.find(".commentsClip").scroll(_.bind(function(){this.$el.trigger("chatWindow:scroll")},this))}this.$el.find("form, .archived").remove();d.is_open?this.model.get("display_name")?this.$el.find(".tip").after(i(d)):this.$el.find(".tip").after(f(d)):this.$el.find(".tip").remove();d.is_archived&&this.$el.prepend('<p class="archived">This is an archived event, comments are no longer being accepted.</p>');
this.model.get("_keep_focus")&&this.$el.find("textarea, input[type=text]").focus();this.delegateEvents();return this},resume:function(){Datajam.debug("resuming");this.model.set({paused:false},{silent:true});this.pollForContent()},submit:function(a){a.preventDefault();this.$el.find("form textarea").eq(0).focus();if(!this.$el.find("form").eq(0).hasClass("disabled"))if(a=this.$el.find("textarea").val()){this.disableSubmit();message=new i.models.Message({text:a});message.url=this.model.get("_submit_url");
message.save(null,{dataType:"json",success:_.bind(function(a){this.$el.find("textarea").val("");a.is_public?this.model.set({interval:500},{silent:true}):this.flash({type:"info",message:"Your message is awaiting moderation."});this.pollForContent()},this),error:_.bind(function(a,b){var d;(d=JSON.parse(b.responseText).errors)&&d.text?this.flash({type:"error",message:"Text "+d.text[0]}):this.flash({type:"error",message:"There was a problem posting your message."})},this),complete:this.enableSubmit})}},
uploadDialog:function(a){a.preventDefault();g(g(a.target).attr("href")).trigger("click")},uploadFile:function(a){g(a.target).val()&&g(a.target).parents("form").ajaxSubmit({dataType:"json",success:_.bind(function(b){if(b&&b.url){var c=g(a.target).parents("form").prev("form").find("textarea");c.val(c.val()+location.protocol+"//"+location.host+b.url);g(a.target).val("");c.focus()}else this.flash({type:"error",message:"Upload failed. Accepted file types are png, jpg, gif."})},this)})}})});h("chat/views/chat_controls",
...
a+"` initializing controls, no model associated."),this}},render:function(){this.$el.html(c);var b={is_open:this.model.get("is_open"),is_archived:this.model.get("is_archived")},a=null;_(this.statuses).each(function(c,e){_.isEqual(c,b)&&(a=e+1)},this);a&&this.$el.find("select").val(a);return this},submit:function(){console.log(this.model.isNew());var b=this.$el.find("select");(b=b.length?b.first().val():null)&&this.model.save(this.statuses[b-1]);return this}})});h("chat/views/incoming_message",["text!chat/templates/message/incoming.html",
"chat/init","chat/views/message","chat/models/message"],function(c){var e=jQuery,b=Datajam.Chat;b.views.IncomingMessage=b.views.Message.extend({events:{"click .approve":"approve","click .reject":"reject"},initialize:function(){_.bindAll(this,"approve","empty","getImages","getLinks","loading","parentModel","parentView","reject","render","_url");this.model||(this.model=new b.models.Message);this.model.bind("change",this.render)},approve:function(a){a.preventDefault();this.loading();this.model.url=this._url();
this.model.set({is_moderated:!0,is_public:!0},{silent:!0});this.model.save().success(_.bind(function(){e("#"+this.$el.attr("id")).remove()},this)).error(function(){alert("There was an error approving this message.")})},empty:function(){this.$el.html("")},loading:function(){this.$el.addClass("loading")},reject:function(a){a.preventDefault();this.loading();this.model.url=this._url();this.model.set({is_moderated:!0,is_public:!1},{silent:!0});this.model.save().success(_.bind(function(){e("#"+this.$el.attr("id")).remove()},
this)).error(function(){alert("There was an error rejecting this message.")})},render:function(){var a=this.model.toJSON(),b=this.parentModel();a.text=this._strip_tags(a.text);a.text=this._linkify(a.text);a.text=this._imgify(a.text);a.text=this._spaceify(a.text);a.text=this._linebreaks(a.text);this.$el.attr("id","message_"+a._id).attr("data-timestamp",a.updated_at);b&&b.get("is_admin")&&this.$el.addClass("sunlight");this.$el.html(_.template(c,a));this.delegateEvents();return this}})});h("chat/views/rejected_message",
["text!chat/templates/message/rejected.html","chat/init","chat/views/message","chat/models/message"],function(c){var e=jQuery,b=Datajam.Chat;b.views.RejectedMessage=b.views.Message.extend({className:"message rejected",events:{"click .approve":"approve","click .deleteAttachments":"deleteAttachments"},initialize:function(){_.bindAll(this,"approve","deleteAttachments","empty","getImages","getLinks","loading","parentModel","parentView","render","_url");this.model||(this.model=new b.models.Message);this.model.bind("change",
this.render)},approve:function(a){a.preventDefault();this.loading();this.model.url=this._url();this.model.set({is_moderated:!0,is_public:!0},{silent:!0});this.model.save().success(_.bind(function(){e("#"+this.$el.attr("id")).remove()},this)).error(function(){alert("There was an error approving this message.")})},deleteAttachments:function(a){a.preventDefault();var b=this.getImages(),c=[];_(b).each(function(a){c.push(_.last(a.split("/")))});c=_(c).compact();c.length&&e.ajax({url:"/chats/upload.json",
type:"POST",dataType:"json",data:{_method:"delete",filenames:c},success:_.bind(function(a){if(a){var c=this.model.get("text");_(b).each(function(a){c=c.replace(a,"")});this.model.url=this._url();this.model.set({text:c});this.model.save()}},this)})},empty:function(){this.$el.html("")},loading:function(){this.$el.addClass("loading")},render:function(){var a=this.model.toJSON(),b=this.parentModel();a.text=this._strip_tags(a.text);a.text=this._linkify(a.text);a.text=this._imgify(a.text);a.text=this._spaceify(a.text);
a.text=this._linebreaks(a.text);this.$el.attr("id","message_"+a._id).attr("data-timestamp",a.updated_at);b&&b.get("is_admin")&&this.$el.addClass("sunlight");this.$el.html(_.template(c,a));this.delegateEvents();return this}})});h("chat/views/moderator_chat","text!chat/templates/chat/show.html text!chat/templates/chat/closed.html text!chat/templates/chat/error.html text!chat/templates/common/flash.html chat/init chat/models/moderator_chat chat/models/message chat/views/incoming_message chat/views/rejected_message chat/collections/moderator_message".split(" "),
...
this._timeout=setTimeout(this.poll,this.model.get("interval"))}},render:function(){var a=this.model.toJSON(),b=_.template(c);this.$el.html(b(a));this.$el.find("h3").eq(0).html(this.model.get("name"));this.$el.find("p.tip").eq(0).remove();return this},renderCollection:function(){var a=this.$el.find(".commentsClip").find(".comments");a.empty();this.collection.each(_.bind(function(b){var d=this.views[b.id];if(!d){d=new (h[this.$el.attr("data-status")])({model:b});this.views[b.id]=d}a.append(d.render().el)},
this));this.updateTopbarBadge()},resume:function(){this.model.set({paused:false},{silent:true});this.poll()},updateTopbarBadge:function(){if(this.collection.url.match("incoming")){var b=a(window.parent.document),c=b.find(".chat-modal[data-chat-id="+this.model.id+"]");if(c.length){var b=b.find(".topbar-nav a[data-controls-modal="+c.attr("id")+"]").eq(0),c=b.find(".badge"),d=this.$el.find("li").length;c.length||(c=b.append(' <span class="badge badge-warning"></span>').find(".badge"));c.html(d);d===
0?c.hide():c.show()}}}})});var j=jQuery;k(["chat/init","chat/views/chat"],function(){j(function(){var c=Datajam.Chat;j(".datajamChatThread").not(".moderator").each(function(){new c.views.Chat({el:j(this)})});j(".datajamChatThread.moderator").each(function(){k(["chat/views/moderator_chat"],_.bind(function(){new c.views.ModeratorChat({el:j(this)})},this))})})})})(jQuery,define,require);
